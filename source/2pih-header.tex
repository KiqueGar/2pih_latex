...
\usepackage[bookmarks=true,unicode=true,pdfborder={0 0 0},
	pdftitle={2pih},
	pdfauthor={NanashiSaito}, breaklinks={true},
...
\usepackage{ragged2e}		% For allignment left or right
\usepackage{wasysym}		% For music symbols on song fragments
\usepackage{siunitx}		% For exponential notation
\usepackage{cjhebrew}		% For hebrew in chapter 2
...
%, ExternalLocation=C:/Users/Kique/Documents/Working/2pih_latex/build/fonts/garamond/
...
%
% TODO: Check if \Star and \Stars are used anywhere, or delete them
...
%
% TODO: Replace this section break with line or similar
\newcommand{\sbreak}{%
    \par
    \SomeVSpace
...
\makechapterstyle{evans}{%
	...
	\ifnum \thechapter < 0		% This is awfully implemented, will try something different later
		\markboth{\MakeUppercase{Strange loops}}
			{\MakeUppercase{Prologue}}%
	\else
...
% TODO: check if a structure like this exists, or delete this section
...
% TODO check if a structure like this exists, or delete this section END
...
\newcommand*{\DatePlace}[1]{\begin{flushleft}\textsc{\textit{#1}}\end{flushleft}\par}

\newcommand*{\DatePlaceInv}[1]{\begin{flushright}\textsc{\textit{#1}}\end{flushright}\par}
											  % Song lyrics
\newcommand*{\song}[1]{\begin{flushright}\eighthnote\twonotes~\textit{#1}~\twonotes\eighthnote\end{flushright}\par}

\newcommand*{\quoteBook}[2]{\begin{quote}\small{#1}\end{quote}\sourceatright{#2}}
%
...
\newenvironment{quotePoem}[1]
 	{	
 		\begin{verse}
 		{	
 			% Authors
			\newcommand{\attrib}[1]{\raggedleft\footnotesize{##1} \normalsize\par}
 			\itshape
 			\small{#1}
 		}
 		\end{verse}
...
\newenvironment{playScript}[2]
	{
		...
		{	
			...
			\newcommand*{\dialogue}[1]{\noindent\flagverse{##1}}	%CHARACTER: .....
			% Character Name + dialogue
			\newcommand*{\sdialogue}[1]{\noindent\scriptsize\flagverse{##1}\small}	%CHARACTER: .....
			% Authors
			\newcommand{\attrib}[1]{\raggedleft\footnotesize{##1} \normalsize\par}	
...
% }{1}}
% TODO: Clean this omake files END
...
Dum-ble-dore thou-sand Huf-fle-puff Huf-fle-puffs Slyther-in Slyther-ins Harry
Flit-wick win-gar-di-um le-viosa Draco Tre-lawney Sev-erus Mal-foy Shatranj}

\newcommand{\UnsetValue}{UNSET}
\newcommand{\CoverAuthor}{TODO: Define Cover author}% Set cover author here, to appear in colophon
\newcommand{\PdfSourceUrl}{github.com/KiqueGar/2pih_latex}% Set URL for the book's source code, to appear in colophon
